...
const PopupNewsletter = () => {
  ...
  useEffect(() => {
    const dontShowAgain = localStorage.getItem('dontShowNewsletter');
    if (!dontShowAgain) {
  ...
  const closePopup = () => {
    if (dontShow) {
      localStorage.setItem('dontShowNewsletter', 'true'); // Store preference in localStorage
    }
    setShowPopup(false);
  };
  ...
  return (
    <>
      {showPopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black-900 bg-opacity-80 p-4">
          <div className=" p-2 relative bg-white-200 rounded-lg overflow-hidden flex flex-col md:flex-row w-full max-w-4xl h-auto md:h-[40vh]">
            ...
            <button
              onClick={closePopup}
              className="absolute top-2 right-2 bg-white text-gray-800 rounded-full p-2 shadow hover:bg-gray-200 focus:outline-none"
            >
              ✕
            </button>
